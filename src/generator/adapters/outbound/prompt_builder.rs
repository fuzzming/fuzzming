...
17. SYNC GHOST STATE ON ACTOR REGISTRATION: When an addActor (or equivalent) function registers a new actor, immediately read that actor's current balance/state from the contract and assign it to the ghost variable. Never assume a newly registered actor has zero balance — they may already hold tokens.\n\
18. INVARIANTS MUST HOLD AT T=0: Every invariant must be true immediately after setUp() completes, before any handler calls. If an invariant can fail on the initial state it is a false positive, not a real bug. Verify your invariant logic against the deployed initial state.\n\
