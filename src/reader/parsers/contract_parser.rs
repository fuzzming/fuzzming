...
pub fn parse_contract(source: &str, _include_comments: bool) -> Result<ContractContext> {
    ...
    for cap in fn_re.captures_iter(source) { if let Some(m) = cap.get(1) { functions.push(m.as_str().to_string()); } }

    let state_re = Regex::new(r"(?:uint256|uint|address|bool|mapping\s*\([^)]*\)|bytes[0-9]*|string)\s+([A-Za-z0-9_]+)\s*;").unwrap();
    let mut state_variables = Vec::new();
    for cap in state_re.captures_iter(source) { if let Some(m) = cap.get(1) { state_variables.push(m.as_str().to_string()); } }

    let mod_re = Regex::new(r"modifier\s+([A-Za-z0-9_]+)\b").unwrap();
    let mut modifiers = Vec::new();
    for cap in mod_re.captures_iter(source) { if let Some(m) = cap.get(1) { modifiers.push(m.as_str().to_string()); } }
