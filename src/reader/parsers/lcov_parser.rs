...
/// Parse lcov content and extract gaps (zero-hit DA and BRDA; zero FNDA for functions)
    ...
    for line in lcov_data.lines() {
        ...
        if let Some(sf) = &current_sf {
            if let Some(cap) = da_re.captures(line) {
                ...
                if hits == 0 {
                    gaps.push(CoverageGap { file: sf.clone(), line: line_no, gap_type: GapType::Line, source_context: Vec::new() });
                }
                continue;
            }
            if let Some(cap) = br_re.captures(line) {
                ...
                if hits == 0 {
                    gaps.push(CoverageGap { file: sf.clone(), line: line_no, gap_type: GapType::Branch, source_context: Vec::new() });
                }
                continue;
            }
            if let Some(cap) = fnda_re.captures(line) {
                let hits: u32 = cap[1].parse().unwrap_or(0);
                let _name = cap[2].trim().to_string();
                if hits == 0 {
                    gaps.push(CoverageGap { file: sf.clone(), line: 0, gap_type: GapType::Line, source_context: Vec::new() });
                }
                continue;
            }
        }

        if line.trim() == "end_of_record" { current_sf = None; }
    }
