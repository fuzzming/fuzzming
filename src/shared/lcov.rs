...
/// Parse raw lcov content into a [`CoverageContext`] with uncovered gaps.
/// Lives in `shared` so both the reader (context enrichment) and the fuzzer
/// (FullCoverage detection) can use it without a cross-component dependency.
